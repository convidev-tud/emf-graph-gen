...
``-x`` | ``--stepwise_export`` | ``false``| Toggle if the processor the variants only once or after each step. The base model generation is not influenced by this toggle. If set to true, a model variant with an edit length of 10 leads to 10 exported models. This operation is not compatible with atomic counting. This operation is not compatible with atomic counting (BOOL).
